...
;; version of free.  The material below might be repeatedly edited or replaced.

(ns free.model
  (:require [free.level :as lvl]
            [free.random :as ran] ; will be clj or cljs depending on dialect
            #?(:clj  [clojure.spec :as s]    ; for clojure spec tests a bottom
               :cljs [cljs.spec :as s])))
...
(def bot-map {:phi 0    ; dummy value: for aljabr but not vectorz needs a number for epsilon-inc on 1st tick (?); immediately replaced by next-bottom
              :epsilon error-u
              :sigma sigma-u
              :theta init-theta
